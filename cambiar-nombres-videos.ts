...
/** Pregunta con reintento si la respuesta no está entre las opciones válidas.
 *  opciones: undefined → acepta cualquier texto
 *  opciones: string[]  → solo acepta esas respuestas (case-insensitive)
 */
function pregunta(texto: string, opciones?: string[]): string {
  while (true) {
    const resp = (prompt(texto) ?? "").trim()
    if (!opciones) return resp
    const lower = resp.toLowerCase()
    if (opciones.includes(lower)) return lower
...
function pedirNombresNuevos(originalesNuevos: Archivo[]): NuevosNombres {
  ...
  while (true) {
    ...
    // Validar que ningún nombre quede vacío tras limpiar
...
async function principal(): Promise<void> {
  ...
  try {
    ...
    // ── Pregunta principal: ¿quieres numeración? ─────────────────────────────
    // Escenario 3 (todos numerados): no tiene sentido preguntar, ve directo a quitar
    if (!hayNuevos) {
    ...
    // Escenarios 1 y 2: hay archivos nuevos → preguntar si quiere numeración
    const quiereNumeracion = preguntaSN(
      "\n🔢 ¿Quieres añadir numeración a los archivos?",
      "s"
    )
 
    // ── Escenario 2: mezcla + quiere numeración → ofrecer quitar antes ───────
    if (hayNumerados && quiereNumeracion) {
      ...
      if (preguntaSN("¿Quieres quitarles la numeración antes de continuar?", "n")) {
        ...
        if (preguntaSN("\n¿Confirmar?", "s")) {
          ...
          // Relanzar con la carpeta limpia
          await principal()
          return
        }
      }
    ...
    // ── Escenario 2: mezcla + NO quiere numeración → avisar que se quitará ───
    if (hayNumerados && !quiereNumeracion) {
      console.log(
    ...
    // ── Sin numeración: quitar la de los ya numerados y renombrar ────────────
    if (!quiereNumeracion) {
      // Quitar numeración a los que ya la tienen
    ...
    while (true) {
      const startResp = pregunta("Número inicial (00,15…23) o 'c' para continuar automáticamente [c]: ")
    ...
    // ── Opción final: quitar numeración (solo si acabamos de numerar) ─────────
